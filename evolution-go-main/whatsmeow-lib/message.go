...
func (cli *Client) handleProtocolMessage(ctx context.Context, info *types.MessageInfo, msg *waE2E.Message) (ok bool) {
	...
	if info.Sender.Device == 0 {
		...
		case waE2E.PeerDataOperationRequestType_FULL_HISTORY_SYNC_ON_DEMAND:
			for _, part := range peerResp.GetPeerDataOperationResult() {
				resp := part.GetFullHistorySyncOnDemandRequestResponse()
				if resp == nil {
					continue
				}
				cli.Log.Infof("Full history sync on-demand response: request=%s code=%s", resp.GetRequestMetadata().GetRequestID(), resp.GetResponseCode().String())
			}
