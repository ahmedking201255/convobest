...
func (cli *Client) preparePeerMessageNode(
	...
	} else if peerReq := message.GetProtocolMessage().GetPeerDataOperationRequestMessage(); peerReq != nil && (peerReq.GetPeerDataOperationRequestType() == waE2E.PeerDataOperationRequestType_HISTORY_SYNC_ON_DEMAND || peerReq.GetPeerDataOperationRequestType() == waE2E.PeerDataOperationRequestType_FULL_HISTORY_SYNC_ON_DEMAND) {
	...
	hasSession, sessionErr := cli.Store.ContainsSession(ctx, encryptionIdentity.SignalAddress())
	if sessionErr != nil {
		return nil, fmt.Errorf("failed to check peer encryption session for %s: %w", encryptionIdentity, sessionErr)
	}
	if !hasSession {
		bundles := cli.fetchPreKeysNoError(ctx, []types.JID{encryptionIdentity})
		bundle = bundles[encryptionIdentity]
		if bundle == nil {
			return nil, fmt.Errorf("failed to establish peer encryption session for %s", encryptionIdentity)
		}
	}
