	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		case "sticker":
			if mimeType != "image/webp" && mimeType != "image/png" && mimeType != "image/jpeg" {
				errMsg := fmt.Sprintf("Invalid file format: '%s'. Only 'image/webp', 'image/png' and 'image/jpeg' are accepted for stickers", mimeType)
				return nil, errors.New(errMsg)
			}
			uploadType = whatsmeow.MediaImage
		case "document":
		...
		case "sticker":
			if isNewsletter {
				return nil, errors.New("stickers are not supported in newsletters")
			}
			media = &waE2E.Message{StickerMessage: &waE2E.StickerMessage{
				URL:           proto.String(uploaded.URL),
				DirectPath:    proto.String(uploaded.DirectPath),
				MediaKey:      uploaded.MediaKey,
