...
func (c *Config) CreateUsersDB() (*gorm.DB, error) {
	if c.SQLiteUsersDB != "" {
		if err := os.MkdirAll(filepath.Dir(c.SQLiteUsersDB), 0751); err != nil {
			return nil, fmt.Errorf("erro ao criar diretório SQLite: %v", err)
		}

		db, err := gorm.Open(
			sqlite.Open(c.SQLiteUsersDB+"?_pragma=foreign_keys(1)&_busy_timeout=5000&_journal_mode=WAL"),
			&gorm.Config{},
		)
		if err != nil {
			return nil, err
		...
		sqlDB, err := db.DB()
		if err != nil {
			return nil, fmt.Errorf("erro ao obter sql.DB do GORM SQLite: %v", err)
		}
...
func Load() *Config {
	...
	sqliteUsersDB := os.Getenv("SQLITE_USERS_DB")
	...
	if sqliteUsersDB == "" && postgresUsersDB == "" && (postgresHost == "" || postgresPort == "" || postgresUser == "" || postgresPassword == "" || postgresDB == "") {
