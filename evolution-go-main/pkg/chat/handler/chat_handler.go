...
type ChatHandler interface {
	...
	FullHistorySyncRequest(ctx *gin.Context)
}
...
// FullHistorySyncRequest requests WhatsApp history for recent days without needing a reference message.
// @Summary Full history sync request
// @Description Full history sync request
// @Tags Chat
// @Accept json
// @Produce json
// @Param message body chat_service.FullHistorySyncRequestStruct true "Chat"
// @Success 200 {object} gin.H "success"
// @Failure 400 {object} gin.H "Error on validation"
// @Failure 500 {object} gin.H "Internal server error"
// @Router /chat/full-history-sync [post]
func (c *chatHandler) FullHistorySyncRequest(ctx *gin.Context) {
	getInstance := ctx.MustGet("instance")
	...
	var data *chat_service.FullHistorySyncRequestStruct
	err := ctx.ShouldBindBodyWithJSON(&data)
	if err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	...
	resp, err := c.chatService.FullHistorySyncRequest(data, instance)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
