...
func (i *instanceRepository) Update(instance *instance_model.Instance) error {
	// Connection settings update an existing instance. Save may fall back to an
	// INSERT when GORM loses the primary-key context, which then violates the
	// unique token constraint and leaves the instance without webhook events.
	...
	if result.RowsAffected == 0 {
		return fmt.Errorf("instance %s was not found during update", instance.Id)
	}
	return nil
}
